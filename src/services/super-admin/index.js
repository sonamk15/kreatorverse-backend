...
class SuperAdminServices {
  createVender = async (venderData) => {
    try {
      ...
      const mailPayload = {
        to: venderData.email,
        html: `<h2>Welcome to the Kreatorverse</h2><p>Login credetial for vender dashboard</p><p>email: ${venderData.email}</p> <p>password: ${venderData.password}</p><p><a href="https://62fa8fd24a1d06129dc7318d--the-awesome-sonamk15-site.netlify.app" target="_blank">Click On </a> this link for login</p>`,
      };
      return await user
        .save()
        .then(async(doc) => {
